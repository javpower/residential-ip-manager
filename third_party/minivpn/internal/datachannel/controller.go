...
func (d *DataChannel) writePacket(payload []byte) (*model.Packet, error) {
	...
	if d.state.dataCipher.isAEAD() || d.sessionManager.TunnelInfo().UsePeerID {
		opcode = model.P_DATA_V2
	}
	packet := model.NewPacket(opcode, d.sessionManager.CurrentKeyID(), encrypted)
	if opcode == model.P_DATA_V2 {
		peerID := d.sessionManager.TunnelInfo().PeerID
		packet.PeerID = model.PeerID{byte(peerID >> 16), byte(peerID >> 8), byte(peerID)}
	}
	return packet, nil
}
