...
func encryptAndEncodePayloadNonAEAD(log model.Logger, padded []byte, session *session.Manager, state *dataChannelState) ([]byte, error) {
	...
	out := &bytes.Buffer{}
	// P_DATA_V2 carries a three-byte peer-id after the opcode. Servers that
	// did not negotiate peer-id use the legacy P_DATA_V1 header instead.
	opcode := model.P_DATA_V1
	if session.TunnelInfo().UsePeerID {
		opcode = model.P_DATA_V2
	}
	if opcode == model.P_DATA_V2 {
		out.WriteByte(dataOpcodeAndKeyHeader(opcode, session))
	}
	if opcode == model.P_DATA_V2 {
...
func opcodeAndKeyHeader(session *session.Manager) byte {
	return dataOpcodeAndKeyHeader(model.P_DATA_V2, session)
}

func dataOpcodeAndKeyHeader(opcode model.Opcode, session *session.Manager) byte {
	return byte((byte(opcode) << 3) | (byte(session.CurrentKeyID()) & 0x07))
}
