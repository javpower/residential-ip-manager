...
func (ws *workersState) moveUpWorker(firstKeyReady <-chan any) {
	...
	// SoftEther can send a keepalive while the TLS worker is still deriving
	// the first data-channel key. Wait before attempting to decrypt it.
	select {
	case <-firstKeyReady:
	case <-ws.workersManager.ShouldShutdown():
		return
	...
	for {
		...
		case pkt := <-ws.muxerToData:
			...
			if len(decrypted) == 16 {
				// SoftEther sends a 16-byte keepalive payload outside the IP stream.
				continue
			}
