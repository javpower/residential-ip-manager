...
func Test_DataChannel_writePacket(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "non-aead without peer id uses P_DATA_V1",
			fields: fields{
				options: &config.OpenVPNOptions{Compress: config.CompressionEmpty},
				session: makeTestingSession(),
...
func Test_DataChannel_writePacketV1WireFormat(t *testing.T) {
	...
	if diff := cmp.Diff(raw, []byte{0x30, 0xaa, 0xbb}); diff != "" {
		t.Errorf("P_DATA_V1 wire format differs: %s", diff)
	}
}
