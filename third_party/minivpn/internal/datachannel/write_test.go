...
func Test_encryptAndEncodePayloadNonAEAD(t *testing.T) {
	...
	// CBC V1 encrypt output is HMAC, IV and ciphertext. Packet.Bytes adds the
	// one-byte P_DATA_V1 header at the wire-serialization boundary.
	goodEncryptedV1, _ := hex.DecodeString("fdf9b069b2e5a637fa7b5c9231166ea96307e4123031323334353637383930313233343581e4878c5eec602c2d2f5a95139c84af")
	// P_DATA_V2 OP32 header (opcode + peer-id) followed by the same payload.
	goodEncryptedV2, _ := hex.DecodeString("48000000fdf9b069b2e5a637fa7b5c9231166ea96307e4123031323334353637383930313233343581e4878c5eec602c2d2f5a95139c84af")
	...
	tests := []struct {
		...
		{
			name: "good P_DATA_V1 encrypt without negotiated peer id",
			args: args{
				logger:  log.Log,
				padded:  padded16,
				session: makeTestingSession(),
				state:   makeTestingStateNonAEAD()},
			want:    goodEncryptedV1,
			wantErr: nil,
		},
		{
			name: "good P_DATA_V2 encrypt with negotiated peer id",
			args: args{
				logger: log.Log,
				padded: padded16,
				session: func() *session.Manager {
					manager := makeTestingSession()
					manager.UpdateTunnelInfo(&model.TunnelInfo{UsePeerID: true})
					return manager
				}(),
				state: makeTestingStateNonAEAD()},
			want:    goodEncryptedV2,
			wantErr: nil,
		},
		{
