...
func customVerifyFactory(pinner authorityPinner) verifyFun {
	...
	customVerify := func(rawCerts [][]byte, verifiedChains [][]*x509.Certificate) error {
		if len(rawCerts) == 0 {
			return fmt.Errorf("%w: no certificates presented", ErrCannotVerifyCertChain)
		}
		// we assume (from docs) that we're always given the
		// leaf certificate as the first cert in the array.
		leaf, err := x509.ParseCertificate(rawCerts[0])
		if err != nil {
			return fmt.Errorf("%w: parse leaf certificate: %v", ErrCannotVerifyCertChain, err)
		}
		intermediates := x509.NewCertPool()
		for _, rawCert := range rawCerts[1:] {
			certificate, err := x509.ParseCertificate(rawCert)
			if err != nil {
				return fmt.Errorf("%w: parse intermediate certificate: %v", ErrCannotVerifyCertChain, err)
			}
